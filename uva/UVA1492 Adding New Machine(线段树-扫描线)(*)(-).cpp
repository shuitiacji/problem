...
思路：先分情况讨论，先选格子向右横着放，那么所有旧机器的左边（包括边界）M-1个格子都不能放，将所有的旧机器向左扩大M-1，答案即为场上还剩下的空白，然后逆向思维用线段树-扫描线求一下被占据的格子总数，再用总数减一下。
记得横放与竖放都算一下，M=1时每一种情况都算了两次要除以二

debug：57,74,77行忘开LL,87行x，y搞反。
